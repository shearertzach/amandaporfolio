...
const Navbar = () => {
  ...
  return (
    <header className='flex justify-between items-center font-pt-sans p-[32px] bg-lightblue shadow-lg'>
      ...
      <nav className="flex gap-[10px] [&>a]:p-[10px] [&>a]:transition-all [&>a]:hover:scale-[115%]">
        ...
        <Link href="/projects" className={`${router.pathname === '/projects' ? 'font-bold font-lg' : ''}`}>Contact</Link>
        <Link href="/contact" className={`${router.pathname === '/contact' ? 'font-bold font-lg' : ''}`}>Resume</Link>
      </nav>
    </header>
  )
}
